...
import { fillSplash } from '../fill-splash.ts';
...
test('inserts a correct PK$SPLASH when the record has none', async () => {
...
test('replaces an incorrect PK$SPLASH with the correct one', async () => {
  const original = await load();
...
test('is a no-op (still valid) for a record whose PK$SPLASH is already correct', async () => {
  const original = await load();
...
test('throws RangeError when the record has no peaks to hash', async () => {
