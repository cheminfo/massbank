...
import { UnrecognizedFieldRule } from '../validation/rules/unrecognized-field-rule.ts';

const rule = new UnrecognizedFieldRule();
const dummyRecord: InternalRecord = { ACCESSION: 'TEST' };

test('should not warn for recognized fields', () => {
...
test('should warn for unrecognized fields', () => {
  const text = `ACCESSION: TEST
...
test('should not warn for comments or empty lines', () => {
  const text = `ACCESSION: TEST
// This is a comment
//
RECORD_TITLE: Test
...
test('should validate method returns empty array', () => {
