...
test('should parse a valid record file', async () => {
...
test('should parse record with multiple CH$NAME fields', async () => {
  const content = await readFile(
...
test('should parse record with annotations', async () => {
  const content = await readFile(
...
test('should parse deprecated records', async () => {
  const content = await readFile(
    join(import.meta.dirname, 'data/MSBNK-test-TST00003.txt'),
    'utf8',
  );
...
test('should throw ParseException for missing ACCESSION', () => {
  const content = 'RECORD_TITLE: Test\n//';

  expect(() => parseRecord(content)).toThrow(ParseException);
});

test('should throw ParseException for invalid format', () => {
  const content = 'ACCESSION: TEST\nINVALID LINE WITHOUT COLON\n//';

  expect(() => parseRecord(content)).toThrow(ParseException);
});

test('should parse all three test files successfully', async () => {
  const files = [
