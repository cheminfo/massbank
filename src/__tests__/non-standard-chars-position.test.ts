...
import { NonStandardCharsRule } from '../validation/rules/non-standard-chars-rule.ts';

test('should report correct line and column for non-standard char in middle of file', () => {
...
test('should report correct line and column for non-standard char on last line', () => {
  const text = `ACCESSION: TEST001
RECORD_TITLE: Test
//™`;
...
test('should skip deprecated records and not report warnings', () => {
  const text = `ACCESSION: TEST001
...
test('should not warn on standard ASCII characters', () => {
  const text = `ACCESSION: TEST001
...
test('should handle non-standard chars in first line correctly', () => {
...
test('should handle allowed special characters without warning', () => {
  const text = String.raw`ACCESSION: TEST001
...
test('should detect non-standard char at exact end of line before newline', () => {
  const text = `ACCESSION: TEST001
RECORD_TITLE: Test™
CH$NAME: Compound
//`;
