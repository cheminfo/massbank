...
- **Rejects a row parsed from a real PK$ANNOTATION table whose source columns the parser did not fully map into typed fields.** The parser's token-count branches are positional but not all of them account for every token: a 3-column row is only fully captured when its third column looks numeric (otherwise the parser reads `[mz, annotation]` and drops the third column — a real shape in lipid nomenclature, e.g. `"494.35 1 [lyso_PC(alkyl-18:0,-)]-"`), and a row of 5 or more columns is always read as `[mz, annotation]` only. Rebuilding from the parsed fields in either case would silently drop the uncaptured column(s). This only applies to rows carrying that raw source text — a caller building a draft by hand cannot trigger it.
...
**Throws:** `RangeError` if a non-empty `PK$PEAK` cannot be hashed (all-zero intensity, a negative intensity, a negative `mz`, or a non-finite `mz`/`intensity`); if a peak's `relativeIntensity` is not finite or is negative; if `ACCESSION` contains a newline or carriage return, is empty or whitespace-only, or has leading or trailing whitespace; if any other field the serializer writes verbatim (or an element of an array-valued one) contains a newline; or if a `PK$ANNOTATION` row cannot survive a round-trip (including a parsed row whose source columns the parser did not fully map into typed fields) — see [Builder API](#builder-api) above for the full legal/illegal combinations
